function initializeConfigs() {
	window.configs = {
		...
		characterBorderWidth: 0.001 * Math.min(window.innerHeight, window.innerWidth),
		...
		// thruster pack
		thrusterPackWidth: 0.06 * Math.min(window.innerHeight, window.innerWidth),
		thrusterPacHeight: 0.09 * Math.min(window.innerHeight, window.innerWidth),
		get thrusterPackX() {
			const floorsXCoordinates = this.floorsXCoordinates;
			const floorXCoordinates = floorsXCoordinates[this.floorsNPieces - 1];
		...
		oxygenPacksXWidth: 0.03 * Math.min(window.innerHeight, window.innerWidth),
		oxygenPacksXHeight: 0.08 * Math.min(window.innerHeight, window.innerWidth),
		oxygenPacksColor: "#FFFFFF",
		oxygenPacksBorderColor: "#FFFFFF",
		oxygenPacksBorderWidth: 0 * Math.min(window.innerHeight, window.innerWidth),
		oxygenPacksSpeed: 0.001 * Math.min(window.innerHeight, window.innerWidth),
		oxygenPacksJittering: 0 * Math.min(window.innerHeight, window.innerWidth),
		...
		floorsBorderWidth: 0.003 * Math.min(window.innerHeight, window.innerWidth),
		get floorsY() {
		...
		get holesSize() {
			return (
				(this.gameCanvasWidth * this.holesWidthProp) / this.floorsNPieces
			);
		},
		get floorsSize() {
		...
		// trees
		get treesHeight() {
			return 0.3 * Math.min(window.innerWidth, this.floorsYProp * window.innerHeight);
		},
		get treesWidth() {
		...
		get treesX() {
			const floorsXCoordinates = this.floorsXCoordinates;
			return floorsXCoordinates.map(
				(coordinates) =>
					coordinates.start +
					0.5 * (coordinates.end - coordinates.start) -
					0.5 * this.treesWidth
			);
		},
		// mountain
		get mountainHeight() {
			return 0.7 * Math.min(window.innerWidth, this.floorsYProp * window.innerHeight);
		},
		mountainColor: "#999999",
		mountainBorderColor: "#FFFFFF",
		get mountainXCoordinates() {
			const floorIdx = Math.round(0.5 * this.floorsNPieces);
			const floorsXCoordinates = this.floorsXCoordinates;
