function initializeConfigs() {
	window.configs = {
		...
		characterOxygenLevelDecrement: 0.04,
		get characterOxygenLevelDecrementJump() {
			return 40 * this.characterOxygenLevelDecrement;
		},
		get characterOxygenLevelDecrementWalk() {
			return 4 * this.characterOxygenLevelDecrement;
		},
		...
		thrusterPackFillDecrement: 1,

		// oxygen pack
		oxygenPacksNpieces: 5,
		oxygenPacksXWidth:
		...
		// platform
		generatePlatformPieceCoordinate(_, idx) {
			const coordinates = {};
			coordinates.start =
				idx * this.floorsSize +
				0.5 * this.holesSize -
		...
		get platformsXCoordinates() {
			...
			const xCoordinates = Array(this.floorsNPieces)
				.fill(0)
				.map(this.generatePlatformPieceCoordinate.bind(this))
				.filter((_, idx) => !unwantedFloorIdxs.includes(idx))
			return xCoordinates;
		},
		get platformsY() {return this.characterY + 0.2*this.characterHeight - this.floorsHeight},
		get platformsYfromFloors() {return this.floorsY - this.platformsY},
		...
		generateTreeX(coordinates) {
			...
			const x = coordinates.start + 0.7 * width - 0.5 * this.treesWidth;
			return x;
		},
		get treesX() {
			const unwantedFloorIdxs = [
		...
		get mountainXCoordinates() {
			...
			const width = floorXCoordinates.end - floorXCoordinates.start;
			const coordinates = {
				start: floorXCoordinates.start + 0.3 * width,
				end: floorXCoordinates.end - 0.3 * width,
			};
			return coordinates;
		},
		get mountainY() {
		...
		// enemies
		generateEnemyParams(coordinates) {
			const width = coordinates.end - coordinates.start;
			const x = Math.random() * width + coordinates.start;
			const speed = (Math.random() * 2 - 1) * this.speedHorizontal;
			const enemyParam = {
				x: x,
				y: this.floorsY - this.enemiesRadius,
				leftBound: coordinates.start,
				rightBound: coordinates.end,
				speed: speed,
			};
			return enemyParam;
		},
		get enemiesParams() {
			const unwantedFloorIdxs = [this.mountainFloorIdx];
			const floorsXCoordinates = this.floorsXCoordinates;
			return floorsXCoordinates
				.filter((_, idx) => !unwantedFloorIdxs.includes(idx))
				.map(this.generateEnemyParams.bind(this));
		},
		get enemiesColor() {
			return this.textColor;
		},
		enemiesRadius: 0.03 * Math.min(window.innerHeight, window.innerWidth),
		...
		get jumpHeight() {
			...
			const tangent = this.speedGravity / this.speedHorizontal;
			const theta = Math.atan(tangent);
			const hypothenuse = this.holesSize / Math.cos(theta);
			let height = hypothenuse * Math.sin(theta);
			height = Math.max(this.platformsYfromFloors, height)
			height = incrementSlack * height;
			return height;
		},
